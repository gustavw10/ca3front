...
function ExternalBackend(){
  ...
  useEffect(() => {
    fetch('http://localhost:8080/startkodeca3/api/info/joke').then(res=>res.json()).then(data=>{
      setJoke(data)
      localStorage.setItem("currentJoke", data.value); 
    })
  }, []);
...
function SaveJoke(props){
  ...
  fetch('http://localhost:8080/startkodeca3/api/info/', {
  method: 'POST',
  headers: {
...
function Webscraper(){
  ...
  const url = "http://localhost:8080/startkodeca3/api/info/parallel";
  useEffect(() => {
